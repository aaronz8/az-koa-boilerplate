...
function generateToken (client, user) {
  ...
  const payload = {
    _client: client._id,
    token: crypto.randomBytes(32).toString('base64') // differentiates accesstoken and refreshtoken
  };

  const options = {
    subject: user._id, 
    issuer: "th3",
    expiresInMinutes: config.get('security:tokenLife')
  };

  return jwt.sign(payload, config.get('security:jwtSecret'), options);
};
